# nerd-deploy-reloaded

> **Maintained fork** of [vscode-deploy-reloaded](https://github.com/mkloubert/vscode-deploy-reloaded) by Marcel Joachim Kloubert. This fork modernizes the toolchain and dependencies to remove the original's known vulnerabilities. Licensed under LGPL-3.0 (unchanged).

[![Open VSX Version](https://img.shields.io/open-vsx/v/eurico-nerd/nerd-deploy-reloaded)](https://open-vsx.org/extension/eurico-nerd/nerd-deploy-reloaded) [![Open VSX Downloads](https://img.shields.io/open-vsx/dt/eurico-nerd/nerd-deploy-reloaded)](https://open-vsx.org/extension/eurico-nerd/nerd-deploy-reloaded)
...
This fork is published on [Open VSX](https://open-vsx.org/extension/eurico-nerd/nerd-deploy-reloaded) (the registry used by Cursor, VSCodium and others) — it is **not** published to the Visual Studio Marketplace.

Launch Quick Open (`Ctrl + P`), paste the following command, and press enter:

```bash
ext install eurico-nerd.nerd-deploy-reloaded
```

Alternatively, download the `.vsix` from the [releases page](https://github.com/eurico-nerd/nerd-deploy-reloaded/releases) and install it via `Extensions: Install from VSIX...` in the command palette.
